...
2. Identify the bounded module(s) to touch instead of editing a single large file:
   ...
   - `oasis/helpers/embedding/`: canonical embed-model normalization/parsing (`normalize_embed_models`, `resolve_embed_models`, `primary_embed_model`, `EmbedModelValueError`); reuse it for CLI parsing and embedding-manager primary model resolution.
...
When changing **any** report preview inside `#report-modal` (vulnerability, executive summary, audit report, future canonical types), follow the **single** architecture described in the `oasis-dashboard-js-patterns` skill (**Report modal architecture** section):
...
- **`oasis/agent/tools.py`**: Thin dispatch — each node calls the matching `SecurityAnalyzer.langgraph_*` method; console UX uses `oasis/helpers/langgraph_cli/` (`langgraph_emit`, `langgraph_emit_phase`, etc.).
...
- **Helpers tied to the graph**:
  - `oasis/helpers/progress/` — executive-summary `phases` rows aligned with LangGraph stages (`ProgressPhaseRowId.GRAPH_*`, `ProgressActivePhase.GRAPH_PIPELINE`) plus scan progress JSON/markdown helpers (keep in sync with report-side progress).
  - `oasis/helpers/langgraph_cli/` — numbered phases, tqdm-safe logging, post-pipeline / report-delivery banners (also used from `report.py`), and canonical vuln-type totals for LangGraph (`embedding_tasks_vuln_types_total`, `deep_payload_vuln_types_total`).
  ...
  - `oasis/helpers/poc/` — compact findings digest JSON for `--poc-assist`, PoC budgets (re-export from config), hints markdown (`build_poc_hints_markdown`), chat options (`poc_assist_chat_options`), stage DEBUG logging (`maybe_debug_log_poc_stage_output`).
...
### Incremental scan progress (see sibling skills)

Details live in the `oasis-python-architecture` skill (constants in `oasis/helpers/progress/__init__.py`, sidecar, helper modules) and the `oasis-dashboard-js-patterns` skill (REST, Socket.IO, stale `updated_at` guard). Touch `oasis/helpers/progress/__init__.py`, `report.py`, `web.py`, and dashboard JS together when the wire contract changes; extend `tests/test_report_schema.py` when behavior is contract-visible.
...
When changing how the assistant validates findings, follow the contract below:
