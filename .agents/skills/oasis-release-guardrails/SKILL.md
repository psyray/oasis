...
- [ ] **Open-source hygiene before tagging/pushing**: grep all reachable history for personal-infrastructure strings (`git log --all -p | grep -E '<hostnames|private URLs|keys>'`) — must be empty on **every branch** (`git branch --contains`); a cleanup commit is not enough, rewrite offending commits (amend / cherry-pick) then `git reflog expire --expire=now --all && git gc --prune=now --aggressive`.
- [ ] Personal infrastructure (server names, private URLs, keys) never appears in committed files — neutral placeholders in examples (`https://llm.example.com/v1`); private server docs stay in the uncommitted an uncommitted private workspace mirror.
