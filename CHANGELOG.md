...
- 🧩 **Consolidated multi-model report (`-rm/--report-model`)**: after a multi-model run (`-m a,b`), findings from every model's canonical JSON are merged by stable fingerprint into `consolidated/consolidated_report.json` (+ Markdown sibling) bucketed by confirmation — confirmed by all / several / single-model, with per-model severities; the report model then synthesizes an executive narrative from a compact digest via structured output (it never re-detects findings; an LLM failure degrades to a narrative-less document). Digest budget: `OASIS_REPORT_CONSOLIDATION_DIGEST_MAX_CHARS` (default 24000). Single-model runs skip the feature (logged).
