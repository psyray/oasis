...
- 🔀 **Independent embedding backend**: chat and embedding workloads are routed to separate backends — embeddings default to native Ollama even when the chat backend targets an OpenAI-compatible server. Flags: `--embed-provider` / `--embed-api-base` / `--embed-api-key` (env `OASIS_EMBED_*`), dashboard RAG follows with `--web-embed-provider` / `--web-embed-api-base` / `--web-embed-api-key` (env `OASIS_WEB_EMBED_*`). Embed model availability checks, chunk-size detection and multiprocessing embedding workers now run on the embedding backend, and the startup log shows both routed backends.
