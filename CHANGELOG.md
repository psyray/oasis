...
- 🤝 **OpenAI-compatible model backend (`--provider openai`)**: run the full pipeline against any OpenAI-compatible local server (vLLM, LiteLLM, LM Studio, llama.cpp, …) via `--provider openai --api-base https://your-server/v1` (optional `--api-key`); structured outputs use `response_format` schemas with schema-in-prompt fallback, context windows via `OASIS_OPENAI_CTX_TOKENS`. Env equivalents: `OASIS_LLM_PROVIDER`, `OASIS_OPENAI_BASE_URL`, `OASIS_OPENAI_API_KEY`.
- 🧱 **Model backend abstraction (`oasis/backends/`)**: provider-agnostic `ModelBackend` with `OllamaManager` (default) and `OpenAICompatManager`, resolved by a `create_model_manager` factory; `oasis.ollama_manager` stays as a compat shim.
- 🔀 **Independent embedding backend**: chat and embedding workloads are routed to separate backends — embeddings **inherit the chat backend configuration by default** (provider, base URL, API key), and explicit flags override to spread the workloads across servers (e.g. a dedicated RAG server). Flags: `--embed-provider` / `--embed-api-base` / `--embed-api-key` (env `OASIS_EMBED_*`), dashboard RAG follows with `--web-embed-provider` / `--web-embed-api-base` / `--web-embed-api-key` (env `OASIS_WEB_EMBED_*`, falling back to the scan-side embedding backend then the chat backend). Every embed model is checked against its backend at startup (`/v1/models` match for OpenAI-compatible servers, local pull for Ollama) — the run aborts with an explicit `Model not available` error otherwise; chunk-size detection and multiprocessing embedding workers run on the embedding backend, the startup log shows both routed backends, and a RAG backend failure at query time shows a ⚠️ notice on the affected assistant answer.
- 🧩 **Consolidated multi-model report (`-rm/--report-model`)**: after a multi-model run (`-m a,b`), findings from every model's canonical JSON are merged by stable fingerprint into `consolidated/consolidated_report.json` (+ Markdown sibling) bucketed by confirmation — confirmed by all / several / single-model, with per-model severities; the report model then synthesizes an executive narrative from a compact digest via structured output (it never re-detects findings; an LLM failure degrades to a narrative-less document). Digest budget: `OASIS_REPORT_CONSOLIDATION_DIGEST_MAX_CHARS` (default 24000). Single-model runs skip the feature (logged). The dashboard lists the report under the `Consolidated` pseudo-model and renders a dedicated JSON modal (TOC, summary buckets, narrative, group tables); the assistant stays off for this kind for now.
- 🧠 **Thinking control on OpenAI-compatible servers**: `-mt` / `-smt` map to vLLM-style `chat_template_kwargs.enable_thinking` — the default (thinking off) no longer lets reasoning burn the `max_tokens` budget; `OASIS_OPENAI_THINKING_KWARGS=auto|on|off`.
- 📡 **Streaming on OpenAI-compatible servers**: `stream: true` is now actually sent — the dashboard assistant no longer returns empty responses on these backends; reasoning deltas render as collapsible thought segments.
- 🌐 **Finding validation: broader language support**: the deterministic catalog now covers **Go, Rust, Kotlin, Scala, Java, Ruby** plus cross-language sinks (C#, Node.js sqlite3/mysql/child-process, JS templates, XML/deserialization); pattern catalog version bumps to **4** (entry-point caches invalidate automatically).
- 🛡️ **Scan-time finding validation (automatic)**: findings are validated after each deep pass and a verdict (`status` / `confidence` / `summary`) is embedded in the canonical reports with a color-coded **status badge**; full payloads persist as a **`finding_validations.json` sidecar** with a flat finding picker, **💬 Ask AI** buttons and Scan-time verdicts in the assistant. Flags: `--validate-findings` / `--no-validate-findings` (default: on), `--validate-findings-budget SEC`, `--validate-findings-narrative`; report schema version bumps to **6**.
- 🧪 **Realistic vulnerable test fixtures**: `test_files/realistic_app/` ships vulnerable mini-apps in Python, C#, Java, JS and PHP, each with safe twin variants to check false positives.
...
- ⏳ **Multi-model scan progress tabs**: the dashboard Scan progress card renders an **Overall** tab (aggregate bar + `X/N models complete`) plus one tab per deep model — emoji + display name, with current (⏳) / done (✓) / pending (grayed) states and per-model phase rows. Tabs auto-follow the model currently scanning and return to Overall when the run completes; `/api/progress` and the `scan_progress` socket event carry additive `models_progress` / `overall` payloads, the latest executive-summary sidecar is selected by its inner `updated_at` within a run (a second model can no longer be shadowed by the first model's "Complete"), and incoming findings refresh the filter option lists and the report view live. Run-level `logs/` directories are no longer mistaken for model directories.
- 🤝 **Consolidated report grouping & context**: findings quoted by different models with different amounts of surrounding context (same vulnerable line) now merge into one group — a deterministic snippet-containment pass on top of the exact fingerprint — instead of splitting into single-model buckets; the consolidated document also carries `project` / `analysis_root` (and the dashboard backfills it for artifacts written before), so its card no longer flags the scanned codebase as unreachable.
- 📦 **Self-hosted dashboard assets & deferred loading**: the socket.io client, marked and Chart.js are bundled locally (`static/js/vendor/`, no CDN dependency — `DASHBOARD_SOCKETIO_CLIENT_URL` default now points at the bundled file), and the dashboard modules ship as ordered `defer` scripts in the page (downloads run in parallel during parsing, execution stays in document order before DOMContentLoaded); the legacy sequential module injector remains only as fallback. Dashboard `onLoad` drops from ~2.3 s to ~0.5 s.
- ⚡ **Dashboard first-load performance**: concurrent report collections are single-flighted (three parallel `force=1` fetches plus the progress monitor share one directory walk instead of re-scanning concurrently — the GIL contention that inflated each call to ~1.5 s is gone), and the first load serves warm in-memory data instead of forcing a re-scan at boot (data is kept fresh by the progress monitor); explicit Reload links keep the forced refresh.

### 🐛 Fixed

- 📄 **`KeyError: 'md'` with JSON-only output**: report post-processing no longer assumes `output_files['md']` exists.
- 🛡️ **Assistant finding validation: PHP support + reliable sink anchoring**: procedural PHP findings are no longer scored `unreachable`; companion fixes for relative `file_path` resolution, ripgrep lookaround fallback, caller-hop taint retry, on-path-only mitigations, and the LangGraph entry-point state key.
- 🖥️ **`-lm` on an unreachable backend**: model listing now fails fast with a clear error instead of printing the misleading default model list.
