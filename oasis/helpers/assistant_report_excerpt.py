...
:mod:`oasis.helpers.assistant_prompt_tuning`.
"""
...
def compact_report_excerpt(report_payload: Dict[str, Any], max_chars: int) -> str:
    ...
    definitions: :mod:`oasis.helpers.assistant_prompt_tuning`):
      - :data:`oasis.helpers.assistant_prompt_tuning.COMPACT_MAX_FILES_LISTED` —
        cap of the ``## Files`` section.
      - :data:`oasis.helpers.assistant_prompt_tuning.COMPACT_MAX_TOP_FINDINGS` —
        cap of the ``## Top findings`` section.
      - :data:`oasis.helpers.assistant_prompt_tuning.COMPACT_SEVERITY_RANK` —
        severity ordering used to keep the worst offenders when trimming.
      - :data:`oasis.helpers.assistant_prompt_tuning.REPORT_COMPACT_TRUNCATION_SUFFIX` —
